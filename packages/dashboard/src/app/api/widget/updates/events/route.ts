...
export async function POST(request: NextRequest) {
  ...
  const admin = await createAdminSupabase(); const { data: project } = await admin.from('projects').select('id,settings,owner_user_id').eq(lookup.column, lookup.value).maybeSingle()
