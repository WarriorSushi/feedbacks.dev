...
import { ArrowRight, Gauge, Github, LockKeyhole, Menu as MenuIcon, Route } from 'lucide-react'
        ...
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 sm:px-6">
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <details className="relative lg:hidden">
              <summary className="inline-flex h-10 w-10 cursor-pointer list-none items-center justify-center rounded-md text-sm font-medium hover:bg-accent sm:w-auto sm:px-2"><MenuIcon className="h-4 w-4 sm:hidden" /><span className="sr-only sm:not-sr-only">Menu</span></summary>
              <nav className="absolute right-0 top-11 z-50 grid min-w-44 gap-1 rounded-lg border bg-popover p-2 shadow-[var(--shadow-float)]" aria-label="Mobile navigation">
                <Link className="flex min-h-11 items-center rounded-md px-3 text-sm hover:bg-accent" href="#product">Product</Link>
                <Link className="flex min-h-11 items-center rounded-md px-3 text-sm hover:bg-accent" href="#setup">Install</Link>
                <Link className="flex min-h-11 items-center rounded-md px-3 text-sm hover:bg-accent" href="#pricing">Pricing</Link>
                <Link className="flex min-h-11 items-center rounded-md px-3 text-sm hover:bg-accent" href="/docs" prefetch={false}>Docs</Link>
                <Link className="flex min-h-11 items-center rounded-md px-3 text-sm hover:bg-accent" href={authHref}>Sign in</Link>
              </nav>
            </details>
          ...
          <div className="mx-auto max-w-6xl px-5 sm:px-6">
            ...
            <div className="grid overflow-hidden rounded-2xl border bg-card md:grid-cols-2">
              <div className="p-6 sm:p-9">
                ...
                <Link href={authHref} className="mt-8 block"><Button variant="outline" className="h-11 w-full">Start free</Button></Link>
              </div>
              <div className="border-t border-primary/20 bg-primary/[0.055] p-6 sm:p-9 md:border-l md:border-t-0">
                ...
                <Link href={proAuthHref} className="mt-8 block"><Button className="h-11 w-full">Start with Pro</Button></Link>
              </div>
            </div>
          </div>
        </section>
          ...
          <div className="mx-auto grid max-w-6xl gap-10 px-5 sm:px-6 lg:grid-cols-[0.8fr_1.2fr]">
            ...
            <div className="divide-y border-y">
              {[
                ...
                <details key={question} className="group py-2"><summary className="flex min-h-14 cursor-pointer list-none items-center justify-between gap-5 py-3 font-semibold">{question}<span aria-hidden="true" className="text-primary transition-transform group-open:rotate-45">+</span></summary><p className="mb-3 max-w-2xl text-sm leading-6 text-muted-foreground">{answer}</p></details>
              ))}
            </div>
          </div>
        </section>
