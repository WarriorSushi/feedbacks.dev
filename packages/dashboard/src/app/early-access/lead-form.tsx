...
export function LeadForm({ open }: { open: boolean }) {
  ...
  if (!open && !result) {
    return (
      <div className="rounded-xl border bg-card/95 p-7 shadow-[var(--shadow-float)]">
        ...
        <p className="mt-3 text-base leading-7 text-foreground/75">Free signup remains open, and the normal Pro-for-free referral programme is available from the dashboard.</p>
  ...
  if (result?.accepted) {
    ...
    return (
      <div className="rounded-xl border border-primary/35 bg-card/95 p-7 shadow-[var(--shadow-float)]">
        ...
        <p className="mt-3 text-base leading-7 text-foreground/75">{placeConfirmed ? 'Your programme place is active.' : <>Sign in with <strong className="text-foreground">{email}</strong> and complete the required guided onboarding. Pro unlocks and your place is confirmed when you finish.</>}</p>
  ...
  return (
    <form onSubmit={submit} noValidate className="space-y-4 rounded-xl border border-foreground/15 bg-card/95 p-5 shadow-[var(--shadow-float)] sm:p-6">
      ...
      <div className="space-y-3.5 border-y border-border py-4">
      ...
      <div className="flex items-start gap-3 text-sm leading-6 text-foreground/75"><LockKeyhole className="mt-0.5 h-4 w-4 shrink-0 text-primary" /><p>Programme reminders are service emails for the benefit you request. Advertising measurement remains controlled separately by your privacy choice.</p></div>
