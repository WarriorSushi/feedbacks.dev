...
import { getOrCreateReferralProgram } from '@/lib/referrals'
...
export default async function InvitesPage() {
  ...
  const program = await getOrCreateReferralProgram(user.id)
  const inviteUrl = `${getMarketingOrigin()}/r/${program.code}`
  const count = Math.min(5, program.successful_referrals)

  return (
    <div className="space-y-6">
      <PageHeader eyebrow="Invite program" title="Give feedbacks.dev a proper introduction" description="You have five invite spots. When five new people create accounts through your link, you receive one complimentary month of Pro." />
      ...
      <section className="overflow-hidden rounded-xl border bg-card shadow-[var(--shadow-soft)]">
        <div className="grid gap-6 p-5 sm:p-6 lg:grid-cols-[1fr_240px] lg:items-center">
          <div>
            ...
            <p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">Share it with product builders who would genuinely use the product. Each verified new account fills one spot.</p>
        ...
        <div className="border-t bg-muted/20 px-5 py-4 sm:px-6">
          {program.reward_granted_at ? (
            ...
            <p className="flex items-start gap-2 text-sm text-muted-foreground"><Gift className="mt-0.5 h-4 w-4 shrink-0 text-primary" /><span><strong className="text-foreground">{5 - count} {5 - count === 1 ? 'signup' : 'signups'} to go.</strong> The one-month reward is added automatically after the fifth verified signup.</span></p>
          )}
        </div>
      </section>
      ...
      <section className="grid gap-4 sm:grid-cols-3">
        ...
        <div className="border-t pt-4"><p className="text-sm font-semibold">New accounts only</p><p className="mt-1 text-xs leading-5 text-muted-foreground">Existing accounts, self-referrals, and duplicate signups do not count.</p></div>
