...
test('landing page explains both sides of the feedback loop and keeps the install-once architecture', () => {
  ...
  assert.match(hero, /That was the whole thing/)
  assert.match(hero, /useReducedMotion/)
  assert.match(hero, /pathLength/)
  assert.match(stories, /Feedback happens inside the product/)
  assert.match(stories, /World-class feedback\. One dashboard/)
  assert.match(stories, /mascot_withlaptop_connected_to_everything\.webp/)
  assert.match(stories, /The useful context arrives with the sentence/)
  assert.match(stories, /Turn a noisy inbox into the next decision/)
  assert.match(stories, /Show users that their feedback shipped/)
  assert.match(stories, /Give good ideas a public place to gather/)
  assert.match(stories, /landing-feature-routing/)
  assert.match(install, /dashboard-generated snippet/)
  assert.match(install, /Keep the stable embed unchanged/)
  assert.match(install, /Copy agent instruction/)
