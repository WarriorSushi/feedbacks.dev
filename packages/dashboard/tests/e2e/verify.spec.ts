...
test('verifies a real product submission and keeps the hosted form as troubleshooting', async ({ page }) => {
  ...
  const setupGroupToggle = page.getByRole('button', { name: 'Collapse Set up project' })
  await expect(setupGroupToggle).toHaveAttribute('aria-expanded', 'true')
  await setupGroupToggle.click()
  await expect(page.getByRole('link', { name: 'Customize form' })).toHaveCount(0)
  await page.getByRole('button', { name: 'Expand Set up project' }).click()
  await expect(page.getByRole('link', { name: 'Customize form' })).toBeVisible()
