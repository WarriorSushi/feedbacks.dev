...
export function WidgetInstallationExperience({ projectId, projectKey, projectName, widgetVersion = DEFAULT_WIDGET_VERSION, initialStep, projectSummary }: WidgetInstallationExperienceProps) {
  ...
  const [setupMode, setSetupMode] = useState<'selection' | 'basic' | 'advanced'>('selection');
  ...
  const handleSetupModeSelect = (mode: 'basic' | 'advanced') => {
    if (mode === 'basic') {
      // Set defaults for basic mode (Inline, standard colors)
      updateConfig({
        embedMode: 'inline',
        primaryColor: '#6366F1',
  ...
  const sections = (
    <div ref={tabsRef} className="space-y-6 sm:space-y-8">
      ...
      <Tabs
        ...
        className="w-full"
      >
        <TabsList className="mb-6 flex w-full gap-2 overflow-x-auto bg-transparent border-0 p-1 pb-3 sm:grid sm:grid-cols-5 sm:gap-2 sm:overflow-visible scrollbar-hide">
          {steps.map((step, index) => (
            <TabsTrigger
              key={step.id}
              value={step.id}
              className="flex-shrink-0 min-w-[80px] h-10 items-center justify-center rounded-full border border-border bg-background px-4 text-xs font-medium transition-all data-[state=active]:border-primary data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-md sm:h-11 sm:w-auto sm:rounded-lg sm:px-2 sm:text-xs lg:text-sm"
            >
              <span className="mr-1.5 opacity-70">{index + 1}.</span> {step.label}
            </TabsTrigger>
          ))}
        </TabsList>
                ...
                {MODE_PRESETS.map((item) => {
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        'relative cursor-pointer rounded-xl border-2 p-4 transition-all hover:border-primary/50 hover:bg-accent',
                        active ? 'border-primary bg-primary/5 ring-2 ring-primary/20' : 'border-muted bg-card'
                      )}
                    >
                      <div className="flex flex-col gap-1">
                        ...
                        {item.helper && (
                          <p className="mt-2 text-[10px] text-muted-foreground/80 font-medium bg-background/50 p-1.5 rounded">
                            {item.helper}
                          </p>
                        )}
                      </div>
          ...
          <Card>
            ...
            <CardContent className={cn(CARD_CONTENT, 'space-y-6')}>
              <div className="grid gap-6 md:grid-cols-2">
                  ...
                  <div className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="relative h-10 w-10 overflow-hidden rounded-full border shadow-sm transition-transform hover:scale-105">
                          <input
                            type="color"
                ...
                <div className="space-y-3">
                  <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Header Style</Label>
                  <div className="space-y-4">
                    <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      <Label className="text-sm font-medium">Accent Icon</Label>
                ...
                <div className="grid gap-6 md:grid-cols-2">
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-muted-foreground tabular-nums bg-muted px-1.5 py-0.5 rounded">{config.spacing || 24}px</span>
                    </div>
                    <input
                      type="range"
                      min={16}
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-muted-foreground tabular-nums bg-muted px-1.5 py-0.5 rounded">{(config.scale || 1).toFixed(2)}x</span>
                    </div>
                    <input
                      type="range"
                      min={0.7}
                  ...
                  {config.embedMode === 'modal' && (
                    <div className="space-y-3 md:col-span-2">
                      <div className="flex items-center justify-between">
                        <Label className="text-sm font-medium">Modal Width</Label>
                        <span className="text-xs text-muted-foreground tabular-nums bg-muted px-1.5 py-0.5 rounded">{config.modalWidth || 480}px</span>
                      </div>
                      <input
                        type="range"
                        min={360}
                        max={720}
                        step={20}
                        value={config.modalWidth || 480}
                        onChange={(event) => updateConfig({ modalWidth: Number(event.target.value) })}
                        className="w-full accent-primary h-2 bg-muted rounded-lg appearance-none cursor-pointer"
                      />
                    </div>
                  )}
        ...
        <TabsContent value="protection" className="space-y-6 sm:space-y-8">
          <Card>
            ...
            <CardContent className={cn(CARD_CONTENT, 'space-y-6')}>
              <div className="space-y-3">
                <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Captcha Provider</Label>
                <div className="grid gap-4 sm:grid-cols-3">
                  {['none', 'turnstile', 'hcaptcha'].map((provider) => {
                    const isSelected = (config.captchaProvider || 'none') === provider;
                    return (
                      <div
                        key={provider}
                        onClick={() => updateConfig({ captchaProvider: provider as any, requireCaptcha: provider !== 'none' })}
                        className={cn(
                          "cursor-pointer rounded-xl border-2 p-4 transition-all hover:border-primary/50 hover:bg-accent",
                          isSelected ? "border-primary bg-primary/5 ring-2 ring-primary/20" : "border-muted bg-card"
                        )}
                      >
                        <div className="flex flex-col items-center gap-2 text-center">
                          <div className={cn("rounded-full p-2", isSelected ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground")}>
                            {provider === 'none' && <ShieldAlert className="h-5 w-5" />}
                            {provider === 'turnstile' && <ShieldCheck className="h-5 w-5" />}
                            {provider === 'hcaptcha' && <Lock className="h-5 w-5" />}
                          </div>
                          <span className="font-semibold capitalize text-sm">{provider === 'none' ? 'Disabled' : provider}</span>
                        </div>
                      </div>
                    );
                  })}
              ...
              {config.captchaProvider === 'turnstile' && (
                <div className="rounded-xl border bg-muted/30 p-4 space-y-4 animate-in fade-in slide-in-from-top-2">
                  <div className="space-y-2">
                    <Label>Turnstile Site Key</Label>
                    <Input
                      value={config.turnstileSiteKey || ''}
                      onChange={(event) => updateConfig({ turnstileSiteKey: event.target.value })}
                      placeholder="0xAAAA..."
                      className="font-mono"
                    />
                    <p className="text-xs text-muted-foreground">
                      Get your site key from the <a href="https://dash.cloudflare.com/" target="_blank" rel="noreferrer" className="underline hover:text-primary">Cloudflare Dashboard</a>.
                    </p>
                  </div>
                </div>
              )}
                ...
                <div className="rounded-xl border bg-muted/30 p-4 space-y-4 animate-in fade-in slide-in-from-top-2">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(event) => updateConfig({ hcaptchaSiteKey: event.target.value })}
                      placeholder="10000000-ffff-ffff-ffff-000000000001"
                      className="font-mono"
                    />
                    <p className="text-xs text-muted-foreground">
                      Get your site key from the <a href="https://dashboard.hcaptcha.com/" target="_blank" rel="noreferrer" className="underline hover:text-primary">hCaptcha Dashboard</a>.
                    </p>
                  </div>
                </div>
              )}
  ...
  if (setupMode === 'selection' && !loading) {
    return <SetupModeSelector onSelect={handleSetupModeSelect} />;
  }
    ...
    <div className="space-y-4 sm:space-y-5 pb-12">
      ...
      <div className="hidden sm:flex sm:items-center sm:justify-between">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="sm" className="-ml-2 h-8 w-8 p-0" onClick={() => setSetupMode('selection')}>
        ...
        <div className="flex items-center gap-2">
          {setupMode === 'basic' && (
            <Button variant="outline" size="sm" onClick={() => setSetupMode('advanced')}>
              <Sparkles className="mr-2 h-4 w-4" /> Customize
            </Button>
          )}
          {setupMode === 'advanced' && (
            <DropdownMenu>
...
function SetupModeSelector({ onSelect }: { onSelect: (mode: 'basic' | 'advanced') => void }) {
  return (
    <div className="grid gap-6 md:grid-cols-2 max-w-4xl mx-auto py-12">
      <div
        onClick={() => onSelect('basic')}
        className="group relative cursor-pointer overflow-hidden rounded-2xl border-2 bg-card p-8 transition-all hover:border-primary/50 hover:shadow-lg"
      >
        ...
        <div className="relative space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Get up and running instantly with our recommended defaults. Perfect for most use cases.
            </p>
          </div>
          <div className="pt-4">
            <ul className="space-y-2 text-sm text-muted-foreground">
              <li className="flex items-center gap-2"><CheckCircle className="h-4 w-4 text-primary" /> Pre-configured inline form</li>
              <li className="flex items-center gap-2"><CheckCircle className="h-4 w-4 text-primary" /> Standard styling</li>
              <li className="flex items-center gap-2"><CheckCircle className="h-4 w-4 text-primary" /> Instant code access</li>
            </ul>
          </div>
          <Button className="w-full mt-4" variant="outline">Select Quick Start</Button>
        </div>
      </div>
      ...
      >
        ...
        <div className="relative space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Full control over every aspect of your widget. Customize appearance, fields, and behavior.
            </p>
          </div>
          <div className="pt-4">
            <ul className="space-y-2 text-sm text-muted-foreground">
              <li className="flex items-center gap-2"><CheckCircle className="h-4 w-4 text-primary" /> Custom branding & colors</li>
              <li className="flex items-center gap-2"><CheckCircle className="h-4 w-4 text-primary" /> Advanced field configuration</li>
              <li className="flex items-center gap-2"><CheckCircle className="h-4 w-4 text-primary" /> Spam protection settings</li>
            </ul>
          </div>
          <Button className="w-full mt-4" variant="outline">Select Custom Setup</Button>
        </div>
      </div>
    </div>
  );
