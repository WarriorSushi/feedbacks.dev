  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
      <div className="container mx-auto px-4 py-4 md:py-8 max-w-3xl">
        <h1 className="text-xl md:text-2xl font-bold mb-2">Feedbacks Widget Demo</h1>
        <p className="text-xs md:text-sm text-gray-600 mb-4 md:mb-6">Loaded from CDN. Use this page to quickly test your API key. Captcha previews shown with test site keys.</p>

        <div className="space-y-4 md:space-y-6">
          {!config && (
            <>
              <div className="border rounded-lg p-3 md:p-4">
                <h2 className="text-sm md:text-base font-semibold mb-2">Inline Embed</h2>
                <div id="feedback-widget" />
              </div>
              <div className="border rounded-lg p-3 md:p-4">
                <h2 className="text-sm md:text-base font-semibold mb-2">Trigger Button</h2>
                <button id="feedback-btn" className="px-3 py-1.5 rounded bg-blue-600 text-white text-sm min-h-[44px] md:min-h-0 flex items-center justify-center">Give Feedback</button>
              </div>
              <div className="border rounded-lg p-3 md:p-4">
                <h2 className="text-sm md:text-base font-semibold mb-2">Floating Button</h2>
                <p className="text-xs md:text-sm text-gray-600">A floating button appears in the bottom-right corner.</p>
              </div>
            </>
          )}
          {config && (
            <div className="border rounded-lg p-3 md:p-4">
              <h2 className="text-sm md:text-base font-semibold mb-2">Preview Using Config</h2>
              {config.embedMode === 'inline' && <div id="feedback-widget" />}
              {config.embedMode === 'trigger' && <button id="feedback-btn" className="px-3 py-1.5 rounded bg-blue-600 text-white text-sm min-h-[44px] md:min-h-0 flex items-center justify-center">Give Feedback</button>}
              {config.embedMode !== 'inline' && config.embedMode !== 'trigger' && (
                <p className="text-xs md:text-sm text-gray-600">A floating button appears in the configured position.</p>
              )}
            </div>
          )}
        </div>
